...
modShallowMysql(){
	...
	sed -ri "/mysql:\/\//s/(mysql:\/\/)[^ ]+(\/.*)/\1$ip:$port\2/" $dest_config_path/*/db.properties
}
...
modData-config-Mysql(){
	...
	sed -ri "/mysql:\/\//s/(mysql:\/\/)[^ ]+(\/.*)/\1$ip:$port\2/" $dest_config_path/*/data-config.xml
}
...
replace1(){
	...
	checkFunctions
	sed -i '$a\load-balance: ip='"$tgtIp"' port=8080' $prj_dir_path/warConfigInfo.ini
	checkFunctions
	#修改配置文件
    modShallowMysql
	modShallowRabbitmq
