...
sed -i '/gridfs-nginx/d' warConfigInfo.ini
sed -i '/mongodb/d' warConfigInfo.ini
echo "gridfs-nginx: ip=$ip port=33000" >>warConfigInfo.ini
echo "mongodb: ip=$ip port=27017" >>warConfigInfo.ini
echo "run docker in $ip"
ssh $user@$ip "docker run --name=pss --net=host --log-opt max-size=1m --log-opt max-file=3 --restart=always -v /etc/localtime:/etc/localtime:ro -v $s_dir1/$config1:/usr/local/nginx-gridfs/conf/nginx.conf -v $s_dir1/logs:/usr/local/nginx-gridfs/logs -v $s_dir2/data/db:/usr/local/mongodb/data/db -v $s_dir2/logs:/usr/local/mongodb/logs -v $s_dir2/$config2:/usr/local/mongodb/etc/mongodb.conf -itd $repository_addr/$service_name:${arr[$tag-1]}"
checkFunctions
