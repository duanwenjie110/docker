...
chooseType()
{
	...
	showall(){
		...
		ssh $user@$targetHostIp "docker ps  -a --format '"'{{.Names}} {{.Status}}'"' " >nameStartStatus.txt
	}
	showall
...
{
	#获取所有主机Ip地址
	dockerNameList=`cat  nameStartStatus.txt|awk '{if($2~/^Exited/) print $1'}`
	...
	then
	        echo "this docker all services started "
		exit -1
	else
		echo "this docker has $ret_dockernamelist stopped"
	fi
	chooseType $ret_dockernamelist
	chooseName="$ret_chooseType"
	echo $chooseName
	ssh $user@$targetHostIp "docker start $chooseName"
	re=$?
	if [ $re -eq 0 ]
	then
		echo "$chooseNames start success"
	else
		echo "$chooseNames start fail"
	fi
